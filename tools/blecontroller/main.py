...
class BleWorker:
    ...
    async def _find_device(self, target: DeviceTarget, timeout: float):
        ...
        SERVICE_UUID = "89d60870-9908-4472-8f8c-e5b3e6573cd1"
        target_name = (target.name or "").strip()
        if not target_name:
            raise RuntimeError("Please enter a device name or address.")

        devices = await BleakScanner.discover(timeout=timeout)
        # Filtra solo i device che pubblicano il SERVICE_UUID
        filtered = []
        for d in devices:
            # d.metadata["uuids"] può essere None o una lista di UUID string
            uuids = d.metadata.get("uuids", []) if hasattr(d, "metadata") else []
            if uuids and SERVICE_UUID.lower() in [u.lower() for u in uuids]:
                filtered.append(d)

        # Cerca tra i device filtrati quello con nome esatto
        for d in filtered:
        ...
        raise RuntimeError(f"Could not find device named '{target_name}' with service UUID {SERVICE_UUID} (timeout={timeout}s).")
