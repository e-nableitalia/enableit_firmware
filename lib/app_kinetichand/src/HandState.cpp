...
void updateServoProgressive()
 {
     pressureValue = PRESSURE_MAX - analogRead(PRESSURE_PIN);
  
  
    if (pressureValue > setpoint) {
        servo_angle = map(setpoint, 0, PRESSURE_MAX, SERVO_MIN, SERVO_MAX);  //limite impostabile
    } else {
        servo_angle = map(pressureValue, 0, PRESSURE_MAX, SERVO_MIN, SERVO_MAX);
    }
...
HandState *HandStateHoldOpening::update(int pressure) {
    if (pressure > PRESSURE_MIN) {
        ...
        DEBUG("Pressure[%d] > PRESSURE_MIN[%d], check hold interval[%d] expired[%d]",pressure, PRESSURE_MIN, delta, INTERVAL_OPEN);
        if (delta > HOLDTIME_INTERVAL_uS) {
            DEBUG("OPEN hold interval expired, start closing hand");
            return HandStateClosing::instance().enter();
            //hand_state = CLOSING;
        }
    } else {
        // discard small muscle contraction and return idle
        DEBUG("Muscle contraction discarded, inteval too small");
...
HandState *HandStateIdleOpen::update(int pressure) {
    if (pressure > PRESSURE_MIN) {
            DEBUG("Pressure[%d] > PRESSURE_MIN[%d], start hold open timer",pressure, PRESSURE_MIN);
...
HandState *HandStateClosed::update(int pressure) {
    if (pressure < PRESSURE_MIN) {
        DEBUG("Check Pressure[%d] < PRESSURE_MIN[%d] to go in IDLE_CLOSE",pressure, PRESSURE_MIN);
        DEBUG("Going in IDLE_CLOSED state");
...
HandState *HandStateClosing::update(int pressure) {
         if (pressure > PRESSURE_MIN) {
              DEBUG("Pressure[%d] > PRESSURE_MIN[%d], check can continue to close hand",pressure, PRESSURE_MIN);
              if (servo_angle < SERVO_MAX) {
                  int magnitude = (pressure / 50) + 1;
                  servo_angle = servo_angle + magnitude;
                  DEBUG("Current servo angle[%d], magnitude[%d]",servo_angle, magnitude);
                  servo.write(servo_angle);
                  return this;
              } else {
                  DEBUG("Current servo angle[%d], hand completely closed",servo_angle);
                  return HandStateClosed::instance().enter();
              }
          } else {
              DEBUG("Pressure[%d] < PRESSURE_MIN[%d], stop closing hand",pressure, PRESSURE_MIN);
              return HandStateClosed::instance().enter();
          }
}
...
HandState *HandStateHoldClosing::update(int pressure) {
          if (pressure > PRESSURE_MIN) {
              DEBUG("Pressure[%d] > PRESSURE_MIN[%d], check can close hand",pressure, PRESSURE_MIN);
              if (m_interval.delta(INTERVAL_CLOSE) > HOLDTIME_INTERVAL_uS) {
                  DEBUG("CLOSE hold interval expired, closing hand");
...
HandState *HandStateOpening::update(int pressure) {
    DEBUG("Opening hand, servo hangle -> [%d]",SERVO_MIN);
...
HandState *HandStateOpen::update(int pressure) {
    DEBUG("Check Pressure[%d] < PRESSURE_MIN[%d] to go in IDLE_OPEN",pressure, PRESSURE_MIN);
    if (pressure < PRESSURE_MIN) {
        DEBUG("Going in IDLE_OPEN state");
        return HandStateIdleOpen::instance().enter();
    } else
        return this;
}
...
HandState *HandStateIdleClosed::update(int pressure) {
    if (pressure > PRESSURE_MIN) {
        DEBUG("Pressure[%d] > PRESSURE_MIN[%d], start hold close timer",pressure, PRESSURE_MIN);
        m_interval.start(INTERVAL_CLOSE);
