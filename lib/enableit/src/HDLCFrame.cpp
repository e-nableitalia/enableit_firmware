...
HDLCFrame::HDLCFrame() : pos(0), len(0), crc(0), escape(false), tx_data_buffer(nullptr), rx_data_buffer(nullptr), channel(0), state(INIT_SYNC), mode(HDLC_IDLE) {}

uint8_t *HDLCFrame::consume(uint16_t &size)
{
    if (mode != HDLCFrame::HDLC_DONE)
    {
        size = 0;
        return nullptr;
    }
    ...
    size = len;
    uint8_t *data = rx_data_buffer;
    rx_data_buffer = nullptr;
    state = INIT_SYNC;
    mode = HDLC_IDLE;
    return data;
}

size_t HDLCFrame::produce(uint8_t *data, uint16_t size)
{
    pos = 0;
    len = size;
    tx_data_buffer = data;
...
bool HDLCFrame::get(uint8_t &data)
{
    switch (state)
    {
    case INIT_SYNC:
    {
        if (data == HDLC_BOUNDARY_MARKER)
        {
            state = SIZE_HIGH;
            pos = 0;
            crc = 0xFFFF;
    ...
    case SIZE_LOW:
    {
        ...
        if (len > 0)
        {
            tx_data_buffer = new uint8_t[len];
            state = DATA;
        }
        else
        {
    ...
    case DATA:
    {
        ...
        else if (data == HDLC_ESCAPE_MARKER)
        {
            escape = true;
            break;
        }
        else if (data == HDLC_BOUNDARY_MARKER)
        {
            state = INIT_SYNC;
            mode = HDLC_ERROR_UNEXPECTED_BOUNDARY;
            return false;
        }
        tx_data_buffer[pos++] = data;
        crc = fcs16(crc, data);
        if (pos == len)
        {
            state = CRC_HIGH;
    ...
    case CRC_HIGH:
    {
        if (data != ((crc & 0xFF) ^ 0xFF))
        {
            state = INIT_SYNC;
            mode = HDLC_ERROR_CRC_MISMATCH;
            return false;
        }
        state = CRC_LOW;
    }
    ...
    default:
        state = INIT_SYNC;
        mode = HDLC_ERROR_UNKNOWN_STATE;
        return false;
    }
    return true;
}

bool HDLCFrame::put(uint8_t &data)
{
    switch (state)
    {
    case INIT_SYNC:
    {
        data = HDLC_BOUNDARY_MARKER;
        state = SIZE_HIGH;
        pos = 0;
        crc = 0xFFFF;
    }
    ...
    case DATA:
    {
        if (escape)
        {
            data = tx_data_buffer[pos++] ^ 0x20;
            crc = fcs16(crc, data);
            escape = false;
        }
        else if (pos < len)
        {
            data = tx_data_buffer[pos++];
            if (data == HDLC_BOUNDARY_MARKER || data == HDLC_ESCAPE_MARKER)
            {
                data = HDLC_ESCAPE_MARKER;
                escape = true;
                pos--; // Do not increment pos, handle it in the next iteration
            }
            crc = fcs16(crc, data);
        }
        else
...
bool HDLCFrame::ready()
{
    return mode == FrameMode::HDLC_DONE && pos == len;
}

HDLCFrame::FrameMode HDLCFrame::getMode()
{
    return mode;
}