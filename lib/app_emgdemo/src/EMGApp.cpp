...
void EMGApp::process() {
    ...
    if (offset_mode)
    {
        ...
        {
            double uV = buffer[channel] / pow(2, 23) * 2.4 * 1000000 - offset;
            average = alpha * uV + (1 - alpha) * average;
            log_d("%d, %d, %f uV, %f uV", channel, buffer[channel], uV, average);
        }
    }
...
void EMGApp::cmdMode()
{
    ...
    else if (mode.equalsIgnoreCase("offset"))
    {
        bool on = parser.getBool(2);
        channel = parser.getInt(3);
        if (on)
        {
            log_d("Starting offset calculation on channel(%d)", channel);
            offset_mode = true;
            average = 0;
        }
        else
        {
            log_d("Stopping offset calculation");
            offset_mode = false;
        }
    }
}
...
void EMGApp::cmdSequence() {
    ...
    {
        int size = parser.getInt(3);
        if ((size <= 0) || (size > 30000))
        {
            log_d("Using default size: 30000");
            size = 30000;
        }
        log_d("Starting capture");
        int count = 0;
        int chunk = 100 * 3;
        uint8_t _buffer[chunk];

        log_d("Disabling all channels");
        for (int i = 0; i < emg.getMaxChannels(); i++)
            emg.disable(i);

        log_d("Enabling channel[%d]", _channel);
        emg.enable(_channel);
        ...
        while (count < size)
        {
            ...
            if (consumed > 0)
            {
                log_d("Consumed[%d]", consumed);
                for (int i = 0; i < consumed; i += 4) // 4 bytes per sample
                {
                    long sample = *((long *)(_buffer + i));
                    // Convert from two's complement
                    if (sample & 0x800000) {
                        sample |= ~0xFFFFFF;
                    }
                    data[count++] = double(sample) / pow(2, 23) * 2.4 * 1000000;
                }
            }
        }
        emg.streaming(false);
        log_d("Capture done, starting dump...");
        EMGFilters filter;
        filter.init(SAMPLE_FREQ_1000HZ, NOTCH_FREQ_50HZ, true, true, true, true);
        for (int i = 0; i < size; i++)
        {
            log_d("%d; %F; %F", i, data[i], filter.update(data[i]));
        }
        log_d("Dump done");
    }
}