...
void EMGApp::process()
{
    ...
    if (offset_mode)
    {
        if (emg.readData(buffer))
        {
            ...
            DBG("%d, %d, %f uV, %f uV", channel, buffer[channel], uV, average);
        }
    }
...
void EMGApp::cmdMode()
{
    ...
    else if (mode.equalsIgnoreCase("offset"))
    {
        ...
        if (on)
        {
            DBG("Starting offset calculation on channel(%d)", channel);
            offset_mode = true;
            average = 0;
        }
        else
        {
            DBG("Stopping offset calculation");
            offset_mode = false;
        }
    }
}
...
void EMGApp::cmdSequence()
{
    ...
    else if (command.equalsIgnoreCase("capture"))
    {
        ...
        DBG("Disabling all channels");
        for (int i = 0; i < emg.getMaxChannels(); i++)
            emg.disable(i);

        DBG("Enabling channel[%d]", _channel);
        emg.enable(_channel);
